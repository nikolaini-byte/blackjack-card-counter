...
class BlackjackDecisionEngine:
    ...
    def get_remaining_cards(self, seen_cards: List[str]) -> Dict[str, int]:
        ...
        """
        remaining = {card: count * self.num_decks for card, count in self.standard_deck.items()}
    ...
    def simulate_dealer_hand(self, dealer_upcard: str, remaining_cards: Dict[str, int]) -> int:
        """
    ...
    def simulate_player_action(self, player_cards: List[str], action: str, 
                             dealer_upcard: str, remaining_cards: Dict[str, int]) -> float:
        """
        ...
        for _ in range(self.simulation_rounds):
            ...
            try:
                ...
                # Simulate dealer
                dealer_value = self.simulate_dealer_hand(dealer_upcard, 
                                                       {k: v for k, v in deck_copy.items() 
    ...
    def calculate_expected_values(self, player_cards: List[str], dealer_upcard: str, 
                                seen_cards: List[str], true_count: float) -> Dict[str, float]:
        """
        ...
        if (len(player_cards) == 2 and 
            self.card_values[player_cards[0]][0] == self.card_values[player_cards[1]][0]):
        ...
        for action in actions:
            ev = self.simulate_player_action(player_cards, action, dealer_upcard, remaining_cards)
    ...
    def get_optimal_decision(self, player_cards: List[str], dealer_upcard: str, 
                           seen_cards: List[str], true_count: float = 0.0) -> Dict:
        """
        ...
        player_value, is_soft = self.calculate_hand_value(player_cards)
        dealer_value = self.card_values[dealer_upcard][0] if dealer_upcard != 'A' else 11
        ...
        return {
            ...
            'confidence': abs(optimal_action[1] - max([ev for action, ev in expected_values.items() if action != optimal_action[0]], default=optimal_action[1])),
            'reasoning': self._generate_reasoning(optimal_action[0], expected_values, player_value, is_soft, dealer_value, true_count)
        }
    
    def _generate_reasoning(self, action: str, expected_values: Dict[str, float], 
                          player_value: int, is_soft: bool, dealer_value: int, 
        ...
        if action == 'hit':
            ...
            reasoning += "Doubling down leverages favorable odds with increased bet size."
        elif action == 'split':
...
def get_decision_recommendation(player_cards: List[str], dealer_card: str, 
                              seen_cards: List[str], true_count: float = 0.0,
                              num_decks: int = 6) -> Dict:
    """
    ...
    return engine.get_optimal_decision(player_cards, dealer_card, seen_cards, true_count)


def calculate_action_probabilities(player_cards: List[str], dealer_card: str,
                                 remaining_cards: Dict[str, int]) -> Dict[str, float]:
    """
