...
class ErrorMessages:
    ...
    INVALID_COUNTING_SYSTEM = (
        "Invalid counting system: {system}. Must be one of: {valid_systems}"
    )
    INVALID_PENETRATION = "Invalid penetration: {penetration}. Must be between 0 and 1."
    INVALID_TRUE_COUNT = (
        "True count must be between {min_count} and {max_count}, got {value}"
    )
    ...
    def validation_error(
        cls, message: str, field: Optional[str] = None, **kwargs: Any
    ) -> Dict[str, Any]:
    ...
    @classmethod
    def not_found_error(
        cls, resource: str, identifier: Any, **kwargs: Any
    ) -> Dict[str, Any]:
