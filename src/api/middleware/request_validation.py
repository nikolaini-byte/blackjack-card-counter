...
from pydantic import BaseModel, field_validator, model_validator, FieldValidationInfo, ValidationError
...
def validate_request_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for key, value in data.items():
            if key in {'decks', 'min_bet', 'max_bet', 'bankroll', 'penetration', 'true_count'}:
                try:
...
async def request_validation_middleware(
    ...
    if any(request.url.path.startswith(p) for p in ("/static", "/docs", "/redoc", "/openapi.json")):
