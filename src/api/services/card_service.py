...
def analyze_cards(card_input: CardInput) -> Union[CardAnalysisResponse, Dict[str, str]]:
    ...
    running_count = counting_systems.calculate_running_count(
        card_input.cards, card_input.counting_system
    )
    ...
    if card_input.true_count is None:
        ...
        true_count = counting_systems.calculate_true_count(
            running_count, decks_remaining, card_input.counting_system
        )
    else:
    ...
    recommendation = _get_play_recommendation(
        player_hand_value, card_input.dealer_card, true_count
    )
    ...
    return {
        "counting_analysis": {
            ...
            "cards_remaining": int(
                card_input.decks * 52 * (1 - card_input.penetration)
            ),
...
def _get_play_recommendation(
    player_hand_value: int, dealer_card: str, true_count: float
) -> ActionType:
    """
