...
async def get_strategy_recommendation(
...
) -> StrategyRecommendation | Dict[str, str]:
    """
    ...
    try:
        ...
        player_hand_value = card_utils.calculate_hand_value(
            strategy_request.player_hand
        )
        dealer_card_value = card_utils.calculate_hand_value(
            [strategy_request.dealer_card]
        )

        # Check for pairs (only if exactly 2 cards)
        is_pair = (
            len(strategy_request.player_hand) == 2
            and strategy_request.player_hand[0] == strategy_request.player_hand[1]
        )
        ...
        logger.error(
            f"Unexpected error in get_strategy_recommendation: {e}", exc_info=True
        )
...
def _generate_alternatives(
    ...
    if action == "HIT":
        ...
        alternatives.append(
            {"action": "STAND", "confidence": remaining_confidence * 0.8}
        )
        ...
        if player_hand_value in [9, 10, 11] and remaining_confidence > 0.1:
            alternatives.append(
                {"action": "DOUBLE", "confidence": remaining_confidence * 0.2}
            )

    elif action == "STAND":
        # For STAND, the main alternative is HIT
        alternatives.append({"action": "HIT", "confidence": remaining_confidence * 0.8})
        # Add SURRENDER as an alternative in certain situations
        if player_hand_value in [15, 16] and dealer_card_value in [9, 10, 11]:
            alternatives.append(
                {"action": "SURRENDER", "confidence": remaining_confidence * 0.2}
            )
