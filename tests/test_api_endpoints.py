...
from src.api.middleware.error_handler import (
    BlackjackError, InvalidCardError, InvalidDeckCountError, InvalidCountingSystemError
)
...
class TestBlackjackAPI(unittest.TestCase):
    ...
    def test_analyze_cards_invalid_card(self):
        ...
        self.assertTrue(any("Invalid card value" in str(err) for err in error_data["detail"]))
    ...
    def test_analyze_cards_invalid_deck_count(self):
        ...
        error_data = response.json()
        self.assertIn("detail", error_data)
        self.assertTrue(any("Number of decks must be between 1 and 10" in str(err) 
    ...
    def test_bankroll_endpoint_success(self):
        ...
        self.assertLessEqual(data["recommended_bet"], self.valid_bankroll_data["max_bet"])
        self.assertGreaterEqual(data["recommended_bet"], self.valid_bankroll_data["min_bet"])
